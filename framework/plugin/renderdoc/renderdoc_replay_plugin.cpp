    ...
    if (!plugin->renderdoc_lib_path.empty())
    {
        ...
        else
        {
            ...
            GFXRECON_LOG_WARNING(
                "Failed to dlopen custom RenderDoc path: %s, error: %s", plugin->renderdoc_lib_path.c_str(), err ? err : "Unknown error");
        }
    }
 ...
 */
static void
ensure_api_loaded_and_start_capture(RenderDocCapturePlugin* plugin, uint32_t event_type, uint64_t frame_index)
{
    ...
    if (plugin->rdoc_api != nullptr)
    {
        GFXRECON_LOG_INFO("Starting RenderDoc capture on event %u (frame_index: %" PRIu64 ")...", event_type, frame_index);
    ...
    switch (event->type)
    {
        ...
        case GFXR_REPLAY_EVENT_FRAME_END:
        {
            ...
            if (plugin->capture_started && event->frame_index == plugin->captured_frame_index &&
                plugin->rdoc_api != nullptr)
            {
                GFXRECON_LOG_INFO("Stopping RenderDoc capture on FRAME_END (frame_index: %" PRIu64 ")...", event->frame_index);
                int result = plugin->rdoc_api->EndFrameCapture(nullptr, nullptr);
