...
static GfxrReplayPluginResult on_event(GfxrReplayPluginV1* self, const GfxrReplayEventHeader* event)
{
    ...
    GFXRECON_LOG_INFO("RenderDoc Plugin received event: type=%u, frame_index=%llu", event->type, event->frame_index);

    if (event->type == GFXR_REPLAY_EVENT_FRAME_BEGIN)
    {
        if (event->frame_index == plugin->target_frame)
        {
